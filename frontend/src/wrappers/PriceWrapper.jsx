...
const PriceWrapper = () => {
  ...
  const handlePriceFilter = async () => {
    dispatch(updateProducts({ products: [], loading: false }));
    const res = await fetchProducts();
    const filtered = await filterProductsByPriceRange(res.data, priceRange.min, priceRange.max);
    dispatch(updateProducts({ products: filtered, loading: true }));
  }
  return (
